...
def refresh_events(service, pdelta, ndelta, calendarFile, outFile):
    ...
    for cal, cal_id in calendar["calendars"]["active"].items():
        eventsResult = (
            service.events()
            .list(calendarId=cal_id, timeMin=start, timeMax=end, singleEvents=True)
            .execute()
        )
...
def main():
    ...
    if flags.events:
        refresh_events(
            ...
            48,
            48,
            os.path.join(os.getcwd(), "gcal/calendar.json"),
            os.path.join(os.getcwd(), "gcal/events.json"),
        )
